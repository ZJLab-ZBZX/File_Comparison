...
def compare_images_SSIM(src_image_path, dst_image_path,threshold=0.95,debug = False,result_output_path=""):
    ...
    # 自动调整至相同尺寸（取最小尺寸）
    max_size = 1024 
    h = min(src_image.shape[0], dst_image.shape[0], max_size)
    w = min(src_image.shape[1], dst_image.shape[1], max_size)
    ...
    if debug:
        # 3. 计算SSIM值和差异图
        score, diff = ssim(src_gray, dst_gray, full=True, win_size=11, data_range=255, eps=1e-6)
